#include "management.h"// 파일명에 따라 바꿔주세염
...
int addNewPerson()
{
	...
	while (1)
	{
		...
		if (strlen(Person[n].personName) < MAX_PERSONNAME)
			break;
		printf("잘못된 형식의 정보를 입력하셨습니다.\n");
	}
	...
	while (1)
	{
		...
		if (Person[n].personAdress[0] != '\0' && Person[n].personAdress[0] != 32 && strlen(Person[n].personAdress)< MAX_ADRESS)
			break;
